...
RSpec.describe ElasticSearch, type: :model do
  ...
  describe "validations" do
   ...
   describe "#update_consul" do
    ...
    let (:model_attributes) { { user: "foobar", password: "asdfasdf", host: "localhost", port: 9200, application_environment: application_environment } }
    ...
    it "should set all the model attributes into consul" do
      ...
      pruned_attributes.each do |k,v|
        expect(Diplomat::Kv.get(application_environment.chroot+'/elastic_search/'+k.to_s)).to eq v.to_s
      end
    end
  end
end
