...
RSpec.describe Postgresql, type: :model do
  ...
  describe "#update_consul" do
    ...
    let(:model_attributes) { { user: "foobar", password: "asdfasdf", host: "localhost", database: "foobar", port: 5432, application_environment: application_environment } }
    ...
    it "should set all the model attributes into consul" do
      ...
      pruned_attributes.each do |k,v|
        expect(Diplomat::Kv.get(application_environment.chroot+'/postgresql/'+k.to_s)).to eq v.to_s
      end
    end
  end
end
